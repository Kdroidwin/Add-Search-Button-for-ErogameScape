// ==UserScript==
// @name         ErogameScape searx.perennialte.ch 検索ボタン追加+Sh
...
// @match        https://sub0000529529.hmk-temp.com/*
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    function addSearchButtons() {
        ...
        let searxParams = "&categories=&language=ja&time_range=&safesearch=0&theme=simple";

        let searchUrls = [
            { url: `https://searx.perennialte.ch/search?q=${encodeURIComponent(title)}+攻略${searxParams}`, label: '攻略' },
            { url: `https://searx.perennialte.ch/search?q=${encodeURIComponent(title)}+おすすめ攻略順${searxParams}`, label: '攻略順' },
            { url: `https://searx.perennialte.ch/search?q=${encodeURIComponent(title)}+聖地${searxParams}`, label: '聖地' }
        ];

        let downloadUrls = [
            { url: `https://sukebei.nyaa.si/?f=0&c=1_3&q=${encodeURIComponent(title)}`, label: 'torrent' },
            { url: `https://www.anime-sharing.com/search/76209205/?q=${encodeURIComponent(title)}&o=relevance`, label: 'DDL' }
        ];
        ...
        function createRow(title, links) {
            ...
            let similarSearchRow =
                [...tbody.querySelectorAll('th')]
                .find(th => th.textContent.includes('類似検索'));
    ...
    function autoAcceptPopup() {
        ...
        let observer = new MutationObserver(() => {
            ...
            let alertButton =
                document.querySelector('button[onclick="ok()"]');
